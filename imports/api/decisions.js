...
Meteor.methods({
  ...
  'decisions.addCollaborator'(email, decId) {
    ...
    if (Meteor.isServer) {
      ...
      if (user && dec) Email.send({from: 'river.kanies@gmail.com', to: email, subject: "You've been invited to collaborate on a ThotPlot!", text: `${user.emails[0].address} has added you as a collaborator on the following decision: ${dec.decision}\n\nView the decision at: http://www.thotplot.com/decisions/${dec._id}`})
    }
  },
